		...
		if (inventory[itemObj] > 0)
        {
            switch (itemObj)
            {
				...
				case "EquipmentFailure":
					inventory[itemObj] -= 1;
					quantity[itemObj].text = (int.Parse(quantity[itemObj].text) - 1).ToString();
/*					for(int i=0;i<appliedOnObject.transform.childCount;i++){
						appliedOnObject.transform.GetChild(i).GetChild(0).GetComponent<Patient>().prob_sev_dec_base = 0.1f;
						appliedOnObject.transform.GetChild(i).GetChild(0).GetComponent<Patient>().prob_sev_inc = 0.07f;
					}*/
